...
def test_optional_platforms_are_disabled_by_default(monkeypatch, config):
    monkeypatch.setattr(
        control_plane.azure_cli,
        "try_run",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("disabled modules must not query Azure")),
    )
    report = control_plane.verify(config)
    assert report.ok
    assert all(not platform.enabled and not platform.present for platform in report.platforms)
    assert all("live:disabled-by-default" in platform.checks for platform in report.platforms)


def test_live_inventory_verifies_both_platforms(monkeypatch, config):
    install_live_azure(monkeypatch, config)
    report = control_plane.verify(config, include_optional=True)
    assert report.ok
...
def test_logic_apps_rejects_extra_top_level_action(monkeypatch, config):
    ...
    install_live_azure(monkeypatch, config, resources=resources)
    approvals = platform_result(control_plane.verify(config, include_optional=True), "contoso-approvals")
    assert not approvals.ok
...
def test_logic_apps_malformed_action_fails_without_crashing(monkeypatch, config):
    resources = live_resources(config)
    resources["contoso-agents-approvals-loop"]["properties"]["definition"]["actions"][
        "Approval_triage_agent"
    ] = "invalid"
    install_live_azure(monkeypatch, config, resources=resources)
    approvals = platform_result(
        control_plane.verify(config, include_optional=True),
        "contoso-approvals",
    )
    assert not approvals.ok
    assert any("not an action object" in error for error in approvals.errors)
