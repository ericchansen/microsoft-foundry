...
from contoso_foundry.data.generate import (
    CASE_SEVERITIES,
    CASE_STATUSES,
    EMPLOYMENT_TYPES,
...
from contoso_foundry.toolbox.tools import Toolbox, ToolError, _validate_arguments
...
def test_every_contract_example_passes_its_parameter_schema() -> None:
    for contract in load_contracts(CONTRACTS_DIR):
        for tool in contract.tools:
            for example in tool.examples:
                _validate_arguments(tool, dict(example.get("arguments", {})))


def test_contract_enums_match_the_generated_closed_vocabularies() -> None:
    contracts = {
        tool.name: tool
        for contract in load_contracts(CONTRACTS_DIR)
        for tool in contract.tools
    }
    assert set(contracts["support_search_cases"].parameters["properties"]["severity"]["enum"]) == set(
        CASE_SEVERITIES
    )
    assert set(contracts["support_search_cases"].parameters["properties"]["status"]["enum"]) == set(CASE_STATUSES)
    assert set(contracts["hr_search_roster"].parameters["properties"]["employment_type"]["enum"]) == set(
        EMPLOYMENT_TYPES
    )
    assert set(contracts["operations_search_work_orders"].parameters["properties"]["status"]["enum"]) == set(
        WORK_ORDER_STATUSES
    )
    assert set(contracts["operations_search_work_orders"].parameters["properties"]["priority"]["enum"]) == set(
        WORK_ORDER_PRIORITIES
    )
...
def test_two_regions_get_different_travel_policies(connection: sqlite3.Connection) -> None:
    ...
    assert emea == {"TPOL-EMEA-01"}
    assert apac_ops == {"TPOL-APAC-02"}


def test_every_caller_gets_exactly_one_server_derived_travel_policy(connection: sqlite3.Connection) -> None:
    for principal in (TRAVEL, OPS):
        policies = toolbox_for(connection, principal).call("travel_get_policy", {})
...
def test_economy_plus_is_permitted_by_an_exception_policy(connection: sqlite3.Connection) -> None:
    toolbox = toolbox_for(connection, OPS)
    fare = toolbox.call("travel_search_fares", {"fare_class": "economy_plus", "limit": 1})[0]
    result = toolbox.call(
        "travel_simulate_booking",
        {
            "route_id": fare["route_id"],
            "fare_id": fare["fare_id"],
            "depart_on": "2026-03-04",
        },
    )
    assert not any("exceeds the permitted maximum" in reason for reason in result["reasons"])


def test_booking_notice_and_round_trip_total_are_enforced(connection: sqlite3.Connection) -> None:
    toolbox = toolbox_for(connection, TRAVEL)
    fare = toolbox.call("travel_search_fares", {"fare_class": "economy", "limit": 1})[0]
    base_arguments = {
        "route_id": fare["route_id"],
        "fare_id": fare["fare_id"],
        "depart_on": "2026-01-06",
    }
    one_way = toolbox.call("travel_simulate_booking", dict(base_arguments))
    round_trip = toolbox.call(
        "travel_simulate_booking",
        {**base_arguments, "return_on": "2026-01-10"},
    )
    assert one_way["decision"] == "requires_approval"
    assert any("advance purchase" in reason for reason in one_way["reasons"])
    assert round_trip["price"] == pytest.approx(one_way["price"] * 2)
...
@pytest.mark.parametrize(
    ...
    [
        ("2026-99-99", None),
        ("2026-03-04", "2026-02-01"),
        ("2025-12-31", None),
    ],
)
def test_booking_simulation_rejects_invalid_or_reversed_dates(
    connection: sqlite3.Connection,
    depart_on: str,
    return_on: str | None,
) -> None:
    toolbox = toolbox_for(connection, TRAVEL)
    fare = toolbox.call("travel_search_fares", {"limit": 1})[0]
    arguments = {
        "route_id": fare["route_id"],
        "fare_id": fare["fare_id"],
        "depart_on": depart_on,
    ...
    if return_on is not None:
        arguments["return_on"] = return_on
    with pytest.raises(ToolError):
        toolbox.call("travel_simulate_booking", arguments)


def test_ordinary_travel_readers_see_only_their_own_bookings(connection: sqlite3.Connection) -> None:
    toolbox = toolbox_for(connection, OPS)
    rows = toolbox.call("travel_list_my_bookings", {"limit": 200})
    assert {row["traveller_employee_id"] for row in rows} <= {toolbox.scope.employee_id}


def test_travel_coordinator_can_read_their_scoped_region(connection: sqlite3.Connection) -> None:
    toolbox = toolbox_for(connection, TRAVEL)
    rows = toolbox.call("travel_list_my_bookings", {"limit": 200})
    assert len({row["traveller_employee_id"] for row in rows}) > 1
...
def test_a_case_can_be_retriaged_within_scope(connection: sqlite3.Connection) -> None:
    ...
    updated = toolbox.call("support_update_case", {"case_id": case["case_id"], "severity": "sev4"})
...
def test_a_case_outside_scope_cannot_be_written(connection: sqlite3.Connection) -> None:
    ...
    with pytest.raises(ScopeViolationError, match="visible to this scope"):
        amer.call("support_update_case", {"case_id": row[0], "severity": "sev1"})
...
def test_a_null_required_argument_is_refused(connection: sqlite3.Connection) -> None:
    toolbox = toolbox_for(connection, SUPPORT)
    with pytest.raises(ToolError, match="cannot be null"):
        toolbox.call("customer_lookup", {"customer_id": None})
...
def test_toolbox_does_not_expose_its_repository(connection: sqlite3.Connection) -> None:
    assert not hasattr(toolbox_for(connection, EMEA), "repository")
...
def test_the_audit_trail_records_calls_without_copying_the_roster(connection: sqlite3.Connection) -> None:
    ...
    assert all(entry.outcome == "success" for entry in toolbox.audit)


def test_the_audit_trail_records_refused_calls(connection: sqlite3.Connection) -> None:
    toolbox = toolbox_for(connection, CONTRACTOR)
    with pytest.raises(PermissionError):
        toolbox.call("hr_search_roster", {"limit": 5})
    assert toolbox.audit[-1].tool == "hr_search_roster"
    assert toolbox.audit[-1].argument_names == ("limit",)
    assert toolbox.audit[-1].outcome == "error:PermissionError"
