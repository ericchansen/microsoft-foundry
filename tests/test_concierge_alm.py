...
def test_solution_pins_one_managed_artifact_for_test_and_prod(repo_root: Path):
    ...
    assert rollback["policy"] == "power-platform-pipeline-redeploy"
    assert rollback["require_allow_older_versions_setting"] is True
    assert rollback["retain_successful_pipeline_deployment"] is True
    ...
    assert rollback["unsafe_redeployment_fallback"] == (
        "forward-fix-or-controlled-uninstall"
    )
...
def test_rollback_uses_supported_pipeline_redeploy_with_data_safeguards(
    repo_root: Path,
):
    ...
    assert "irreversibly overwrites" in runbook
    assert "target-environment backup" in runbook
    assert "forward fix" in runbook
    assert "Never substitute an arbitrary old ZIP import" in runbook
...
def test_concierge_sources_contain_no_live_identifiers_or_secrets(repo_root: Path):
    ...
    for target in targets:
        for finding in scan.scan_path(target).findings:
            if (
                finding.rule == "guid"
                and (repo_root / "solutions" / "ContosoConcierge" / "src") in finding.path.parents
            ):
                continue
