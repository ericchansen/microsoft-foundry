...
from contoso_foundry.research.workflow import AGENT_VERSION, REQUIRED_CONTRACT_VERSIONS, build_research_graph
...
def test_evaluations_fail_on_wrong_agent_version(repo_root: Path, tmp_path: Path) -> None:
    ...
    with pytest.raises(EvaluationError, match="routes version"):
...
def test_hosted_configuration_routes_exact_versions(repo_root: Path) -> None:
    ...
    assert "CONTOSO_RESEARCH_VERSION" in unified
