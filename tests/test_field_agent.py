...
def test_pydantic_agent_calls_the_canonical_work_order_tool(tmp_path: Path) -> None:
    def model(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        del info
        if len(messages) == 1:
            return ModelResponse(parts=[ToolCallPart("lookup_work_order", {"work_order_id": "WO-00015"})])
        ...
        return ModelResponse(parts=[TextPart("WO-00015 is a completed priority access-point installation.")])
    ...
    try:
        ...
        result = asyncio.run(agent.run("Summarize WO-00015.", deps=FieldDependencies(toolbox)))
        assert result.output == "WO-00015 is a completed priority access-point installation."
...
def test_pydantic_agent_retries_a_malformed_tool_identifier(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert [call.tool for call in toolbox.audit] == [
            "operations_lookup_work_order",
            "operations_lookup_work_order",
        ]
        assert [call.outcome for call in toolbox.audit] == [
            "error:ToolError",
            "success",
        ]
    finally:
