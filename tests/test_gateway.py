...
def test_gateway_config_has_safe_project_defaults(repo_root: Path):
    ...
    assert {
        ...
        if deployment.deployment_name != "travel-gpt-5-4-mini"
    } == {"Microsoft.DefaultV2"}
...
def test_live_verifier_accepts_the_complete_control_plane_contract(repo_root: Path, monkeypatch):
    ...
    status = gateway.collect_status(
        ...
        config,
        expected_location="northcentralus",
    )

    assert status.ok
    assert status.verified_model_deployments == (
        "contoso-field-model",
        "gpt-5.4-mini-2026-03-17",
        "support-gpt-5-4-mini",
        "travel-gpt-5-4-mini",
...
def test_connection_inventory_rejects_an_unexpected_gateway_path(repo_root: Path):
    config = gateway.load_config(repo_root / "config" / "gateway.yaml")
    live = _live_contract(config)
    unexpected = {
