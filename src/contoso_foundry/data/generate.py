...
def _employees(seed: SeedInputs, rng: random.Random, locations: list[Row]) -> list[Row]:
    ...
    def make(index: int, region: str, department: str, title: str, manager_id: str | None) -> Row:
        # Names are drawn from the cross product without replacement. Walking it
        # deterministically keeps the draw count fixed no matter how many
        # employees are requested.
        slot = index
        while True:
            candidate = f"{given[slot % len(given)]} {family[(slot // len(given)) % len(family)]}"
            if candidate not in used_names:
                break
            slot += len(given) * len(family)
    ...
    for region in REGIONS:
        ...
        for other in REGIONS:
            ...
            for destination in sorted(by_region[other], key=lambda s: s["location_id"]):
                pairs.append((hub, destination))
                pairs.append((destination, hub))
...
def _travel_bookings(
    ...
    for index in range(int(seed.config["volumes"]["travel_bookings"])):
        ...
        wants_exception = traveller["department"] in {"Field Operations", "Customer Support"}
