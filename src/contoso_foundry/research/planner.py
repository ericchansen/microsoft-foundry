...
def plan_question(question: str) -> list[dict[str, Any]]:
    ...
    product_id = re.search(r"\bPROD-\d{4}\b", question, re.IGNORECASE)
    ...
    if any(term in text for term in ("product", "catalog", "catalogue")):
        return [_step("catalog_search_products", is_discontinued=0, limit=50)]
    ...
    if any(term in text for term in ("customer", "account", "segment")):
        ...
        for phrase, value in segments.items():
            if phrase in text:
