...
class Toolbox:
    ...
    @property
    def repository(self) -> ScopedRepository:
    ...
    def call(self, name: str, arguments: dict[str, Any] | None = None) -> Any:
        ...
        cleaned = _validate_arguments(tool, dict(arguments or {}))
        ...
        self._audit.append(
            ToolCall(
                ...
                argument_names=tuple(sorted(cleaned)),
    ...
    def _travel_get_policy(self, _: dict[str, Any]) -> list[dict[str, Any]]:
        # No parameters at all: the applicable policy follows from the resolved
        # region, so there is nothing for a caller to express a preference about.
        return self._repository.list_rows("travel_policies", filters=None, limit=10)
    ...
    def _travel_simulate_booking(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        policies = self._repository.list_rows("travel_policies", filters=None, limit=10)
        if not policies:
            raise ScopeViolationError("no travel policy applies to this scope")
        policy = policies[0]

        reasons: list[str] = []
        fare_ranking = {"economy": 1, "premium": 2, "business": 3, "first": 4}
        ...
        return {
            ...
            "return_on": arguments.get("return_on"),
