...
1. deploy a backend and project connection whose release name is bound to the
   agent definition major version, without changing the previous version's backend;
...
6. only then place that version and an exact image digest in the disabled job,
   without redeploying the validated backend.
