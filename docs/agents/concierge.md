...
> **Code boundary:** the repository carries tenant-neutral source and an ALM
> contract. Live environments, capacity, connections, imports, and publication
> remain separately authorized prerequisites and are never inferred from code.
...
- Synthetic contract tests demonstrate expected delegation and persona
  ...
  the published runtime before a PROD-eligible artifact exists. Repository tests
  do not substitute for that live acceptance.
- Copilot Credit capacity must be verified through an authorized source. Missing,
  forbidden, or empty evidence means capacity is unknown, not sufficient.
