...
> **Optional module:** disabled by default. When explicitly enabled, the current
> GA `Microsoft.App/agents@2026-01-01` resource API creates an isolated,
> review-only agent inside the project-owned resource group.
...
The deployment consumes the same derived `location` as the rest of the platform.
Before enabling this optional module, `foundry regions --enable-module
optional-control-plane` requires that location to appear in the current
...
SRE Agent remains a disabled-by-default unpublished-price module in the [cost
model](../platform/costs.md). Microsoft documents Azure Agent Unit
consumption, but no matching USD Retail Prices API meter exists. The project
therefore preserves its reserve rather than inventing a price or recording zero.
