...
Any future backend must reject direct anonymous or key-based access before the
route is enabled. The template defines that contract without claiming that an
undeployed backend already satisfies it.
