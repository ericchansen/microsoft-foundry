...
**Share-alike or non-permissive corpora — fetched, never committed.** Google's
...
`data/external/manifest.yaml` pins the repository, commit and licence, and a fetch
script pulls it into a gitignored directory on demand. Anyone can reproduce the
evaluation; nobody inherits an obligation by cloning.
...
- name: schema-guided-dialogue
  ...
  vendored: false          # fetched at evaluation time, never committed
  destination: data/external/sgd/   # gitignored
```

`vendored: false` is load-bearing. The provenance gate asserts that anything
marked non-vendored has **no bytes** under version control, so the promise is
...
| Dialogue evaluation corpus | [Schema-Guided Dialogue](https://github.com/google-research-datasets/dstc8-schema-guided-dialogue) | CC-BY-SA-4.0, fetched not vendored |
