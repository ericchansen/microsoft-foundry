...
| Travel traffic | `contoso-agents-traffic-env`, `contoso-agents-travel-traffic`, `contoso-agents-travel-traffic` identity | Container Apps environment, disabled-by-default synthetic traffic job, and its identity |
| Travel ledger | `synthetic-traffic-ledger` | Blob container enforcing bounded synthetic traffic claims |
...
| Optional SRE | `contoso-agents-sre-control-plane`, `contoso-agents-sre` | **Optional**, disabled-by-default SRE Agent and identity |
| Optional Approvals | `contoso-agents-approvals-loop`, `contoso-agents-approvals` | **Optional**, disabled-by-default Logic Apps agent loop and identity |
...
!!! warning "Publication readiness evidence: live Field path blocked"

    On 2026-08-31 UTC, the SDK inventory returned zero `contoso-field`
    external-agent records. No fresh smoke or correlation/span count was
    produced because the exact live ownership gate failed on declared-resource
    and role-assignment drift, including an unset
    `SRE_OPERATOR_GROUP_OBJECT_ID`. The app remained unchanged on
    `contoso-field--0000001`: internal ingress, minimum zero/maximum one, zero
    replicas, and a digest-pinned image; no resource-group deployment was
    active. Direct `az containerapp exec` failed as expected because there was
    no replica, and no Azure write was made. Re-run the gates below before each
    presentation; until all pass, use the deterministic Field segment and do
    not claim live telemetry or registration.
...
Run the smoke only inside an already-running replica of the current Container
App revision. Connecting with `az containerapp exec` is the documented
...
The console path also requires an active replica. Scale-to-zero can legitimately
leave zero replicas, in which case `az containerapp exec` fails. Do not change
the replica minimum ad hoc during a presentation. A planned maintenance smoke
may temporarily wake the app only after the live boundary is clean, with the
original minimum recorded and restored in `finally`; until a repository
operation owns that rollback, use the deterministic Field fallback below.
Never make ingress external.
...
The checked-in registration command is intentionally write-authorized and must
not be run casually during the presentation. It first requires a clean live
ownership boundary and exact fresh telemetry:
...
On the current checked-in implementation, successful completion creates an
external-agent version and prints its name plus the verified nonzero span count
and `gen_ai.agent.id`. It is not a read-only check. An authorized operator must
read the result back through the SDK and require the exact name and OTel ID
before describing registration as verified. The official portal creation path,
...
az resource show --resource-group rg-contoso-agents `
  --resource-type Microsoft.CognitiveServices/accounts/projects `
  --parent "accounts/contoso-agents-foundry" `
  --name travel --query "{Name:name,State:properties.provisioningState}" `
  --output table
```
...
4. Select a trace from the demo time window. A good trace has a root Travel
   request or smoke span, model activity, `contoso.travel.tool` child spans, and
...
Prompt and completion content recording is privacy-sensitive. The repository
keeps content capture disabled for the external and hosted services and uses
only synthetic prompts for Travel. Microsoft documents
...
| --- | ---: | --- | --- |
| Support | +5 min | **Public-preview hosted agent; code and deterministic evaluation are durable; live use requires the protected deployment gate** | Three-stage Agent Framework workflow, server-bound support identity, `foundry support evaluate`, exact-version/100%-route verification. See [Support](agents/contoso-support.md) and [hosted agents](https://learn.microsoft.com/azure/foundry/agents/concepts/hosted-agents). |
| Research | +5 min | **Public-preview hosted agent; code and mocked deterministic evaluation are durable; live deployment is permission/capacity dependent** | LangGraph planner-retrieval-synthesis, request-local audit, exact route/model/toolbox versions. See [Research](agents/research.md) and [LangGraph traces](https://learn.microsoft.com/azure/foundry/how-to/develop/langchain-traces). |
...
| Control Plane | +5 min | **Optional and disabled by default; SRE/Approvals inventory is permission and deployment dependent** | `foundry platform-inventory --no-live`, optional declarations, SRE review-only posture, Approvals synthetic human-review contract. See [Control Plane coverage](operations/control-plane-coverage.md) and [Foundry Control Plane](https://learn.microsoft.com/azure/foundry/control-plane/how-to-manage-agents). |
