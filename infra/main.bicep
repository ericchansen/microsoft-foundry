...
resource projectWorkspaceLogAnalyticsReaders 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for (projectName, index) in projectNames: {
  name: guid(logAnalyticsWorkspace.id, foundryProjects[index].id, logAnalyticsReaderRoleId)
...
resource projectWorkspacePrivilegedMonitoringReaders 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for (projectName, index) in projectNames: {
  name: guid(logAnalyticsWorkspace.id, foundryProjects[index].id, privilegedMonitoringDataReaderRoleId)
  scope: logAnalyticsWorkspace
  properties: {
    principalId: foundryProjects[index].identity.principalId
    principalType: 'ServicePrincipal'
